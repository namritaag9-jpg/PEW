...
def customer_track_api():
    """Public customer lookup by job number or serial number.

    Every successful customer lookup is recorded in scan_events so the
    admin tracking-history page can show what was scanned and when.
    """
    value = (request.args.get("q") or "").strip()[:80]
    ...
    c.execute("""
        ...
        (job_id, serial, scan_type, scanned_by,
         scanned_at, scanned_date, device)
        VALUES (?, ?, ?, ?, ?, ?, ?)
    """, (
        job["id"],
        job["serial"],
...
@app.route('/reports/customer-tracking')
@login_required
def customer_tracking_history():
    ...
    rows = c.execute("""
        ...
        WHERE se.scan_type = 'customer_tracking'
    ...
    return render_template(
        'customer_tracking_history.html',
        rows=rows
    )
